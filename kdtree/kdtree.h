 ...
 * Copied from mp_mosaics with minor alteration (Point is a key, int is the value)
 *
 * @author Zesheng Wang
...
class KDTree
{
  ...
  public:
     ...
     */
    bool smallerDimVal(const Point<2>& first, const Point<2>& second,
                       int curDim) const;
     ...
     *  Point::operator<().
     */
    bool shouldReplace(const Point<2>& target, const Point<2>& currentBest,
    ...
    /**
     * Default constructor for KDTree.
     *
     * @param other The KDTree to copy.
     */
    KDTree();
     ...
     */
    int findNearestNeighbor(const Point<2>& query) const;
